...
x_values = [0.01, 0.02, 0.04, 0.05, 0.06, 0.08, 0.1] # Nlayer_values
print(f"GPU configurations to process: {x_values}")

# Base path
base_path = Path(__file__).parent / "seed_1758632847/tillicum/vs_LearnRate"
...
def process_and_save_data():
    ...
    for i, x in enumerate(x_values):
        # Format learning rate for path: 0.01 -> 01, 0.02 -> 02, 0.1 -> 10, etc.
        lr_str = f"{int(x*100):02d}"
        dict_path = base_path / f"DeepHall_n{N}l{int(2*Q)}_NN{N_layer}L_rate0p{lr_str}"
        ...
        if csv_path.exists():
            try:
                ...
                processed_data[x] = {
                    ...
                    'config_name': f"LearnRate = {x}"
                }
                
                print(f"Successfully processed: LearnRate0p{lr_str}")
                
            except Exception as e:
                print(f"Error processing LearnRate0p{lr_str}: {e}")
        else:
            print(f"File not found: {csv_path}")
    
    # Save processed data
    data_file = base_path / f"plotdata_EcVSrate_n{N}l{int(Q*2)}.pkl"
...
def load_processed_data():
    ...
    existing_data_file = base_path / f"plotdata_EcVSrate_n{N}l{int(Q*2)}.pkl"
...
def create_plot(colormap_name='plasma'):
    ...
    plt.title(f"batch size = {batchsize}, NN layers = {N_layer}, vs. learning rate (ν=1/3)", fontsize=18)
    ...
    # Save the figure
    output_path = base_path / f"EcVSrate_n{N}l{int(Q*2)}.png"
