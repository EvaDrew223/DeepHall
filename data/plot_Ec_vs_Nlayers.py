...
x_values = [2, 3, 4, 5] # Nlayer_values
print(f"GPU configurations to process: {x_values}")

# Base path
base_path = Path(__file__).parent / "seed_1758632847/tillicum/vs_Nlayer"
...
def process_and_save_data():
    ...
    for i, x in enumerate(x_values):
        dict_path = base_path / f"DeepHall_n{N}l{int(2*Q)}_NN{x}L_{batchsize}GPU8"
        ...
        if csv_path.exists():
            try:
                ...
                processed_data[x] = {
                    ...
                    'config_name': f"NN{x}L"
                }
                
                print(f"Successfully processed: NN{x}L")
                
            except Exception as e:
                print(f"Error processing NN{x}L: {e}")
        else:
            print(f"File not found: {csv_path}")
    
    # Save processed data
    data_file = base_path / f"plotdata_EcVSNlayer_n{N}l{int(Q*2)}_{batchsize}.pkl"
...
def load_processed_data():
    ...
    existing_data_file = base_path / f"plotdata_EcVSNlayer_n{N}l{int(Q*2)}_{batchsize}.pkl"
...
def create_plot(colormap_name='plasma'):
    ...
    plt.title(f"batch size = {batchsize}, vs. NN layers (ν=2/5)", fontsize=18)
    ...
    # Save the figure
    output_path = base_path / f"EcVSNlayer_n{N}l{int(Q*2)}_{batchsize}.png"
    plt.savefig(str(output_path), bbox_inches='tight', dpi=300, transparent=True)
